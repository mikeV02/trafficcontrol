...
Acl::init(char const *filename)
{
  ...
  // Backport from 10.x: reset anonymous blocking flags on reload
  ...
  // Backport from 10.x: load anonymous blocking rules
...
///////////////////////////////////////////////////////////////////////////////
// Backport from 10.x: Parse the anonymous blocking settings
bool
...
Acl::eval(TSRemapRequestInfo *rri, TSHttpTxn txnp)
{
  ...
  if (result.found_entry) {
    ...
    if (nullptr != entry_data_list) {
      ...
      // Backport from 10.x: check anonymous blocking.
      // When anonymous blocking is active and no country/regex rules are
      // configured, default_allow was forced to false by loadallow() returning
      // true (due to allow.ip entries). In that case ret is already false for
      // everyone at this point, so we reset it to true here and let
      // eval_anonymous() bring it back to false only for anonymous IPs.
      // When country rules ARE active we leave ret as-is so that country
      // decisions are preserved: anonymous blocking can only deny, not allow.
      if (_anonymous_blocking) {
        if (allow_country.empty() && allow_regex.empty() && deny_regex.empty()) {
          ret = true;
          TSDebug(PLUGIN_NAME, "Anonymous blocking active with no country rules, defaulting to allow before anon check");
        }
...
///////////////////////////////////////////////////////////////////////////////
// Backport from 10.x: Returns true if entry data contains an
// allowable non-anonymous IP. False otherwise.
bool
