...
protected:
  ...
  // --- Backport from 10.x: anonymous blocking fields ---
  ...
  bool    loadanonymous(const YAML::Node &anonNode); // Backport from 10.x
  bool    eval_country(MMDB_entry_data_s *entry_data, const char *path, int path_len);
  bool    eval_anonymous(MMDB_entry_s *entry_data);  // Backport from 10.x
